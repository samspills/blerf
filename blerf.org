...
** TODO Fs2 Streaming Experiments: Mapping
:PROPERTIES:
:EXPORT_FILE_NAME: stream-exp-1
:END:

I had a small crisis of faith in my own understanding of Fs2 Streams, and particularly how
data/chunks moves through the various combinators. I wrote many small experiments to convince myself
of what was happening, and they could be useful more generally so I am sharing them here. To remove
some of the suspense, I was confused about `prefetchN` so that's where we're headed.

*** The Setup
As with previous experiments, I'm using scala-cli so that it is maximally easy for someone to
run and iterate themselves. Each experiment is a self-contained function and I just updated which I
called.

The setup for each stream is fairly similar and looks like:
#+begin_src scala
Stream
  .emits(0 until 100)
  .chunkN(10)
...
To break it down:
- ~emits(0 until 100)~ gives me a ~Stream~ of integers, 0 to 99
- ~chunkN(10)~ rechunks the stream so I have chunks of 10 integers each
- I'm left with a ~Stream[Pure, Chunk[Int]]~ but I want to work with a ~Stream[Pure, Int]~. Calling
  ~uncunks~ flattens the stream back into the type I want, but maintains the underlying chunk
  structure I created.

*** ~map~ and ~mapChunk~
Lets start at the beginning, by comparing ~map~ and ~mapChunk~
